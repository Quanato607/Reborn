...
class Brats2018_divide(Dataset):
    ...
    def __init__(self, json_path, data_root, crop_size=[160, 192, 128], modes=['flair', 't1', 't1ce', 't2'] , train=True, normalization = True, dataset='brats', pretrain=False):
    ...
    def _load_item(self, idx: int):
        ...
        mask = MASKS_TEST[mask_id]             # True/False * 4

        volumes = []
        modes = list(self.modes) + ['seg']
        p = "_" if self.dataset == 'brats' else '_'
        for mode in modes:
            volume_path = os.path.join(self.data_root, patient_id, patient_id + p + mode + '.nii')
        ...
        # ---------- 拼接掩码通道 ----------
        mask_tensor = torch.tensor(mask, dtype=torch.float32).view(4, 1, 1, 1)   # [4]
