...
## Main section
task_name = 'brats_1_smunet_pretrained_gaco_dot1'
    ...
    for epoch in range(num_epochs):
        ...
        for batch_id, batch in tqdm(enumerate(loaders['pretrain']), total=len(loaders['pretrain']), desc=f"Pretrain Epoch {epoch+1}"):
            ...
            for mask in all_masks:
                ...
                # === KD / 全模态引导 loss ===
                seg_f, style_f, content_f = model_full(x_f)
                seg_m, style_m, content_m = model_missing(batch_xn)

                # 计算 KD 损失
                loss_dict = losses_kd['co_loss'](config, seg_f, content_f, y_f, seg_m, content_m, style_f, style_m, epoch)
                loss_dict['loss_Co'].backward(retain_graph=True)  # 保留计算图以便后续对抗训练
